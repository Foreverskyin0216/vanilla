...
export const addMessage = async (threadId: string, content: string, role: 'user' | 'assistant') => {
  return new OpenAI().beta.threads.messages.create(threadId, { content, role })
}
...
export const chat = async (threadId: string, humanMessage: BaseMessage, aiMessage?: BaseMessage) => {
  ...
  if (answer) {
    ...
    await openai.threads.messages.create(threadId, { content: question, role: 'user' })
  }
  ...
  const message = messages.data[0].content[0] as { text: { value: string } }

  return message.text.value
}

export const useTools = async (tools: DynamicStructuredTool[], messages: BaseMessage[]) => {
  return new ChatOpenAI({ model: 'gpt-4o-mini', temperature: 0 }).bindTools(tools).invoke(messages)
}
...
export const retrieveInfoFromWebPage = async (link: string, question: string) => {
  ...
  return chain.invoke({ question, context: retrieved })
}
